...
<<<<<<< HEAD
	...
	TPrimitiva *arboles[15], *bancos[12], *semaforos[7];


//POSICIONES
    for(int i=0; i<15; i++)
    {
        arboles[i] = new TPrimitiva(ARBOL_ID, ARBOL_ID);
        arboles[i]->tx = posArboles[i][0];
        arboles[i]->tz = posArboles[i][1];
    }
    arboles[8]->ty = 8.0;

    for(int i=0; i<15; i++)
        escena.AddObject(arboles[i]);

    for(int i=0; i<12; i++)
    {
        bancos[i] = new TPrimitiva(BANCO_ID, BANCO_ID);
        bancos[i]->tx = posBancos[i][0]+8;
        bancos[i]->tz = posBancos[i][1]-37;
        bancos[i]->ry = posBancos[i][2];
        escena.AddObject(bancos[i]);
    }

    for(int i=0; i<8; i++)
    {
        semaforos[i] = new TPrimitiva(SEMAFORO_ID, SEMAFORO_ID);
        semaforos[i]->tx = posSemaforos[i][0];
        semaforos[i]->tz = posSemaforos[i][1];
        semaforos[i]->ty = 0;
        semaforos[i]->ry = posSemaforos[i][2];
        escena.AddObject(semaforos[i]);
    }
